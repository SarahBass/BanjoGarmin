...
function timerCallback() as Void {
    myCount += 1;
    if (myCount >= 150) {  
    // Stop after 150 counts (~60 seconds with 400ms interval)
    // Stop after 300 counts (~60 seconds with 200ms interval)
            myCount = 0;   
    }
...
function onLayout(dc as Dc){
    ...
    myTimer.start(method(:timerCallback), 400, true);  // Reference the timerCallback function correctly
...
/*                 _       _     _           
  ...
  \ \ / / _` | '__| |/ _` | '_ \| |/ _ \/ __|
   ...
   var talk = ["KNOCK KNOCK", "Banjo - BARK BREATH IS HERE -", "Mumbo busy, sweep many leaves -", "Brother - Someone is HERE -", "Great- To use the watch PRESS A", "Chimpy like ORANGES", "Clanker Has STATS", "This be Gruntildas BOOTY", "Well done mortal...", "SNAP SNAP"] as Array<String>;
   var talkStats = ["Heres your STATS ", "Calm down, GEEKY, well get your STATS", " Too hot for STATS. Mumbo want sun tan.", "I think they want their STATS", "Your STATS are below, Good Luck-", "Chimpy give STATS", "On Clankers BACK", "Read your STATS if you DARE", "Here are your STATS", "THESE ARE YOUR STATS"] as Array<String>;  
    //Banjo, Kazoie, Mumbo, Tootie, Bootles, Chimpy, Clanker, Treasure, Alligator, Grunty
    /*----------Battery------------------------------*/ 
    var userBattery = "0";
    var batteryMeter = 1;
 ...
 /*     _                           _            _    
   ...
   //Draw Background
    var water= waterPhase(userSTEPS, myCount);
    water.draw(dc);
    //Draw Sprite
...
if (System.getDeviceSettings().is24Hour || hours/10> 0){
    ...
    dc.drawText(centerX+14,(230*screenHeightY)+venus2XL-4,funFont,( monthArray[today.month]+" "+ today.day +" " +today.year), Graphics.TEXT_JUSTIFY_CENTER );
  ...
  if (System.getDeviceSettings().screenHeight <370){
   if (userSTEPS>9999){
    if (today.sec%2==0){
     dc.drawText(centerX+30, 275*screenWidthX, wordFont, "Here are your STATS, you filthy RATS", Graphics.TEXT_JUSTIFY_CENTER );}
     else{dc.drawText(centerX+30, 275*screenWidthX, wordFont, "Now LEAVE before I send the CATS", Graphics.TEXT_JUSTIFY_CENTER );}
   }else{
     if (today.sec%2==0){
    dc.drawText(centerX+30, 275*screenWidthX, wordFont, talk[userSTEPS/1000], Graphics.TEXT_JUSTIFY_CENTER );}
     else{dc.drawText(centerX+30, 275*screenWidthX, wordFont, talkStats[userSTEPS/1000], Graphics.TEXT_JUSTIFY_CENTER );}
}}
else{
     if (userSTEPS>9999){
    if (today.sec%2==0){
     dc.drawText(centerX, 275*screenWidthX, wordFont, "Here are your STATS, you filthy RATS", Graphics.TEXT_JUSTIFY_CENTER );}
     else{dc.drawText(centerX, 275*screenWidthX, wordFont, "Now LEAVE before I send the CATS", Graphics.TEXT_JUSTIFY_CENTER );}
   }else{
     if (today.sec%2==0){
    dc.drawText(centerX, 275*screenWidthX, wordFont, talk[userSTEPS/1000], Graphics.TEXT_JUSTIFY_CENTER );}
     else{dc.drawText(centerX, 275*screenWidthX, wordFont, talkStats[userSTEPS/1000], Graphics.TEXT_JUSTIFY_CENTER );}
}
}

    dc.drawText(centerX+10,(230*screenHeightY)+venus2XL,funFont,( monthArray[today.month]+" "+ today.day +" " +today.year), Graphics.TEXT_JUSTIFY_CENTER );
    dc.drawText( 125 *screenWidthX,300*screenHeightY , funFont,  ("^"), Graphics.TEXT_JUSTIFY_CENTER );
    dc.drawText( 125 *screenWidthX,330 *screenHeightY, wordFont,  userHEART, Graphics.TEXT_JUSTIFY_CENTER );
    dc.drawText( centerX, (300)*screenHeightY, funFont,  ("$"), Graphics.TEXT_JUSTIFY_CENTER );
    dc.drawText( centerX, (330)*screenHeightY, wordFont,  (userSTEPS), Graphics.TEXT_JUSTIFY_CENTER );
    dc.drawText( 235 *screenWidthX,300*screenHeightY , funFont, "!", Graphics.TEXT_JUSTIFY_CENTER );
    dc.drawText( 235 *screenWidthX,330*screenHeightY , wordFont, userCAL, Graphics.TEXT_JUSTIFY_CENTER );
...
function dogPhase(seconds, steps){
  ...
  var venus2X = 43*(screenHeightY/360);
  //var venus2X = (screenWidthX)-((seconds%20)*20);
  var venus2Y = 253*(screenHeightY/360);
...
//DrawOpacityGraphic - dog -
function waterPhase(steps, seconds){
  var screenHeightY = System.getDeviceSettings().screenHeight;
  //var screenWidthX = System.getDeviceSettings().screenWidth;
 ...
 // var venus2Y = 15;
  ...
  (new WatchUi.Bitmap({
    ...
    (new WatchUi.Bitmap({
        :rezId => Rez.Drawables.water10,
        :locX => venus2X,
        :locY => venus2Y
    })),
   (new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water11,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water12,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water13,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water14,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water15,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water16,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water17,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water18,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water19,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water20,
    :locX => venus2X,
    :locY => venus2Y
})),
(new WatchUi.Bitmap({
    :rezId => Rez.Drawables.water21,
    :locX => venus2X,
    :locY => venus2Y
}))
];
if (steps>9999){
return waterArray[20+(seconds%2)];
}
else{
return waterArray[((steps/1000)*2)+(seconds%2)];  
}
}
